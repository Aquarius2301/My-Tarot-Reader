...
export const useGetAiHistoryReadings = () =>
  useQuery({
    queryKey: GET_AI_HISTORY_READINGS_KEY,
    queryFn: historyApi.getAiHistoryReadings,
  });

export const useDeleteAiHistory = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (historyId: string) => historyApi.deleteAiHistory(historyId),
    onSuccess: () => {
